...
import com.epgis.mapsdk.activities.style.CircleLayerActivity;
...
import com.epgis.marker.InitMarkerActivity;
 ...
 * 2020-10-28
 */
public class InitManageActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        mapView.getMapAsync(aegisMap -> {
            ...
            // 移图到指定位置
            aegisMap.setCameraPosition(new CameraPosition.Builder().target(targetPos).zoom(7.0f).build());
            // 设置地图显示样式
            aegisMap.setStyle(Style.STREETS);
            //初始化打点图标
            initDefaultIconMarker(aegisMap);
            aegisMap.setOnMarkerClickListener(new AegisMap.OnMarkerClickListener() {
                @Override
                public boolean onMarkerClick(@NonNull Marker marker) {
                    /*marker.setSnippet("12321");
                    marker.setIcon(IconFactory.getInstance(InitManageActivity.this).fromResource(R.drawable.blue_circle));
                    marker.setDraggable(true);
                    aegisMap.updateMarker(marker);*/
                    LatLng latLng = new LatLng(marker.getPosition().getLatitude(), marker.getPosition().getLongitude());
                    String point="("+marker.getPosition().getLatitude()+","+marker.getPosition().getLongitude()+")";
                    Toast.makeText(InitManageActivity.this,point+"被点击了",Toast.LENGTH_LONG).show();
    ...
    private void initDefaultIconMarker(AegisMap aegisMap) {
        aegisMap.setInfoWindowAdapter(new AegisMap.InfoWindowAdapter() {
            ...
            public View getInfoWindow(@NonNull Marker marker) {
                return new Button(InitManageActivity.this);
            }
        });
        ...
        for(Map map:mapList){
           ...
           /* markerOptions.getMarker().setIcon(IconFactory.getInstance(InitManageActivity.this).fromResource(R.drawable.blue_circle));
            ...
            markerOptions.setIcon(IconFactory.getInstance(InitManageActivity.this).fromResource(R.drawable.blue_circle));
            markerOptions.title("测试点");
            markerOptions.setSnippet("我被点击了");
        ...
        if (mapView != null) {
            mapView.getMapAsync(aegisMap -> {
                if (!isAdded) {
                    ...
                    markerOptions.icon(IconFactory.getInstance(InitManageActivity.this).fromResource(R.drawable.select_point));
                    // 默认传入坐标为思极坐标
                    ...
                    marker.setDraggable(true);
                    marker.setIcon(IconFactory.getInstance(InitManageActivity.this).fromResource(R.drawable.miami_beach));
                    aegisMap.updateMarker(marker);
                    marker.setIcon(IconFactory.getInstance(InitManageActivity.this).fromResource(R.drawable.blue_circle));
                    aegisMap.updateMarker(marker);
                    // 移动到当前点位置
